...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{grafiken/yang2025.drawio.png}
    \caption{TODO: yang2025.drawio.png}
    \label{fig:yang}
\end{figure}
...
Xiang u. a. (2021)\\\midrule
         ...
         \textbf{Symbolische Repräsentation} (T1 bis T5)&  \multicolumn{2}{c}{T4 (ungeordnet, extrahiert pfadrelevante Informationen, ignoriert Reihenfolge)}\\\midrule
...
Likelihood-Ratio-Framework &Korrelationsmatrix und spektrales Clustering \\\midrule
         ...
         \textbf{Datenbanken}&  \multicolumn{2}{c}{VISION und EVA-7K}\\\midrule
...
TODO: Zusammenfassung der Genauigkeit?

% Die empirische Leistungsfähigkeit quantitativer Modelle wird durch exzellente Erkennungsraten belegt. Der an \textit{Bag-of-Words} angelehnte Ansatz erreicht in Kombination mit Entscheidungsbäumen eine Balanced Accuracy von 98,5\,\% bei der allgemeinen Integritätsprüfung zwischen originalen und manipulierten Videos; die spezifische Identifikation der verwendeten Manipulationssoftware gelingt mit einer Genauigkeit von 97,6\,\% \cite{yang_efficient_2020} [1, 2]. Die Weiterentwicklung durch \textit{Type-aware}-Algorithmen steigert diese Präzision und erzielt sowohl bei der Zuordnung zu sozialen Netzwerken als auch bei der Identifikation der Bearbeitungssoftware durchschnittliche F1-Werte von 0,99 \cite{xiang_forensic_2021} [3, 4]. Die aktuelle Meta-Analyse von Yang u.\,a. (2025) bestätigt diese Resultate und fasst zusammen, dass quantitative Verfahren bei der Identifikation von Software und Netzwerken nahezu fehlerfrei arbeiten (F1 und Accuracy > 0,99), während die spezifische Identifikation einzelner Quellkameramodelle (\textit{Source Camera Identification}) weiterhin methodisches Verbesserungspotenzial aufweist \cite{yang_comprehensive_2025}.
...
%Die forensische Wirksamkeit pfadbasierter Analysen zeigt sich insbesondere bei der Erkennung subtiler Manipulationen und der Markenzuordnung. Das Likelihood-Ratio-Modell von Iuliani u.\,a. (2019) erzielt bei der Integritätsprüfung eine perfekte Trennschärfe (AUC = 1,0) für Videos, die über soziale Netzwerke geteilt oder ohne Re-Encoding (mittels FFmpeg) geschnitten wurden \cite{iuliani_video_2019} [8, 9]. Selbst bei minimalen Metadaten-Manipulationen (wie durch ExifTool) erreicht das Verfahren bei 70\,\% der getesteten Geräte einen AUC-Wert von über 0,82 und bei der Markenidentifikation fast durchgehend einen Wert von 1,0 \cite{iuliani_video_2019} [9, 10]. Die pfadbasierten Clustering-Ansätze von Ramos-López u.\,a. (2020) belegen zudem, dass unüberwachte Dichte-Algorithmen (wie OPTICS) Videos effektiv ohne vorheriges Training nach Herstellermarken gruppieren können \cite{ramos_lopez_digital_2020} [11, 12]. Dabei werden auf großen forensischen Datensätzen Rand-Index-Werte von 0,89 (VISION-Datensatz) bis 0,94 (ACID-Datensatz) erreicht \cite{ramos_lopez_digital_2020} [12, 13].
...
\begin{itemize}
    \item T1 und T2 reagiert stark auf minimale Strukturveränderungen
    \item  T3 und T4, sind aber fehleranfällig bei identischen Boxen \cite{yang_comprehensive_2025}
    \item Bei Re-Muxing kann oft kein Rückschluss mehr auf Originalkamera (SCI) gewonnen werden, da die Boxen überschrieben werden \cite{li_learning_2024}
\end{itemize}
